    ...
    $.widget( "apex.codeEditor", {
        ...
        _create: function() {
            ...
            if ( o.usePlainTextArea ) {
                ...
                this._editor.on( "change", function( event, data ) {

                    // Use CodeMirror 'change' event to simulate a keypress on text area, allowing DA 'Key press' to work
                    self.element.children( "textarea" ).trigger( "keypress", event, data );
                });

                this._editor.on( "blur", function( event, data ) {

                    // Trigger blur on the textarea, so DA 'blur' will work
                    self.element.children( "textarea" ).trigger( "blur", event, data );

                    // Simulate 'change' event (on blur and value changed), so DA 'change' will work
                    if ( self._editor.getValue() !== self.oldValue ) {
                        self.element.children( "textarea" ).trigger( "change", event, data  ); 
                    }
                });
            }
        ...
        _eventHandlers: {
            ...
            focusin: function( event, data ) {
                this.element.addClass( C_ACTIVE );
                if ( !this.options.usePlainTextArea ) {
                    this.oldValue = this._editor.getValue();
                }
            },

...
function sc_getResults(sessionId) {
  ...
  if ($v("P1003_SQL_COMMAND1") == "")
    return false;
  var useSQLCommandWidget = getCodeEditorWidget().
    codeEditor("option", 'usePlainTextArea') == false;
  sc_getSel(useSQLCommandWidget);
  // set P1003_SQL_COMMAND2 = P1003_SQL_COMMAND2.Selected text
...
function getCodeEditorWidget(){
  var widget = $("#P1003_SQL_COMMAND1_widget");
  return widget;
}

/**
 * Gets the internal element used as a the code editor.
 * Could be either the code editor plugin or a simple Text Area.
...
function getCodeMirrorInstance(){
  ...
  var codeMirror = widget.find('.CodeMirror').get(0).CodeMirror;
  return codeMirror;
}
...
function sc_getSel(useSQLCommandWidget) {
  var cmd, end, start;

  /**
   * This parameter specifies whether we should take the selection from
   * using the P1003_SQL_COMMAND1 CodeEditor plugin or just the
   * regular text area.
   * 
   * Default: false. Uses the text area.
   */
  if(typeof useSQLCommandWidget == 'undefined'){
    useSQLCommandWidget = false;
  }

  // Adding support for the P1003_SQL_COMMAND1 with the CodeEditor plugin.
  // if the code editor is set to 'text area mode' useSQLCommandWidget will
  // be undefined
  if(useSQLCommandWidget){

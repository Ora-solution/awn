...
*/
    ...
    $.apex.propertyEditor.addPropertyType( PROP_TYPE.TEMPLATE_OPTIONS, {
        ...
        _getProperties: function( pProperty$, prop ) {
            return templateOptionsHelper.getProperties(this.templateOptions, this.getValue( pProperty$ ).split( ":" ), prop.metaData.isReadOnly, prop);
        },
        ...
        _getDisplayValue: function( prop ) {
            ...
            this.templateOptions = model.getTemplateOptions( gLastComponents[ 0 ].getProperty( prop.propertyName ) );

            if ( isEmptyVal && gLastComponents.length > 1 ) {
 ...
 */
     ...
     */
    function _prepareMultiSelectMap( pComponents, pPropertyId ) {
        var i,
            j,
            lProperty,
            lPropertyValue,
        ...
        for ( lCountKey in occurrenceCounts ) {
            if ( occurrenceCounts.hasOwnProperty( lCountKey ) ) {
                if ( occurrenceCounts[ lCountKey ] === componentsCount ) {
                    isDetermined = true;
                } else {
                    isDetermined = false;
                }
    ...
    function _getValueAfterMultiSelect( pMultiSelectValues, pProperty  ) {
            ...
            if ( isCheckbox ) {
                if ( isVaries ) {
                    ...
                    if ( pProperty._originalValBeforeMultiSelect ) {
                        val = _getValueAfterMultiSelect( lValues, pProperty );
                    } else {
                        val = lValues;
                    }
                }
            ...
            if ( $.inArray( lPropertyMetaData.type, TYPES_TO_CHECK_LOV_VALUES ) !== -1 ) {
                ...
                if ( isVaries ) {
                    if ( isCheckbox || isTemplateOption ) {
                        lPropertyMetaData.multiSelectData = _prepareMultiSelectMap( pComponents, pProperty.id );
                    }
                }
            ...
            change: function( pEvent, pData ) {
                var i,
                    ...
                    lMessage = model.transaction.message( {
                        ...
                        component:  gLastComponents[ 0 ],
                        property:   gLastComponents[ 0 ].getProperty( propertyName ),
                ...
                for ( i = 0; i < gLastComponents.length; i++ ) {
                    lProperty = gLastComponents[ i ].getProperty( propertyName );

                    if ( lProperty._indeterminateValues ) {
                        lValueToSave = _getValueAfterMultiSelect( lPropertyValue, lProperty );
                    } else {

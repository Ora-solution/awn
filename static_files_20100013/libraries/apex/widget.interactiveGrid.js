    ...
    $.widget( "apex.interactiveGrid",
                        ...
                        if ( !$.isFunction( apex.widget.colorpicker ) ) {
                            $.when(
                                $.getScript( apex_img_dir + "libraries/jquery-colorpicker/1.4/js/" + 
                                    ( debug.getLevel() === 0 ? "apex.colorpicker.min.js"            : "apex.colorpicker.js"   ) ),
                                $.getScript( apex_img_dir + "libraries/apex/" + 
                                    ( debug.getLevel() === 0 ? "minified/widget.colorpicker.min.js" : "widget.colorpicker.js" ) ),
            ...
            if ( pViewId ) {
                ...
                if ( this.viewsImpl[ pViewId ].isConfigured() ) {
                    ...
                    if ( lConfig.toolbar && lConfig.toolbar.searchField && lConfig.toolbar.columnSelection ) {
                        this.getToolbar().toolbar( "findElement", "column_filter_button" ).prop( DISABLED, !lHasTextFilterableColumns );
                    }
                    ...
                    this._toggleEnableAction( lSupports.singleRowView && lConfigViewFeatures.singleRowView, "single-row-view" );
                    ...
                    lMenuItem = this._toolbarFind( "selection_submenu" );
                    if ( lMenuItem ) {
       ...
       // todo use css to add the '...'
        ...
        _updateSelectionActions: function() {
            ...
            if ( lConfig.editable && lThisView.supports.edit && !lThisView.singleRowMode ) {
                if (lThisView.view$.grid( "option", "selectCells" )) {
                    // In range selection mode
                    lSelectedRecordIds = lThisView.view$.grid( "getSelectedRange" ).recordIds;
                    lEnabled = this._isCurrentlyEditable() && lSelectedRecordIds.length !== 0;
                    // todo consider if don't want to allow fill, clear, copyDown on row selection then need to disable based on selection mode
                    this._toggleEnableAction( lEnabled, ["selection-fill", "selection-clear", "selection-revert"] );
                    this._toggleEnableAction( false, ["selection-duplicate", "selection-refresh", "selection-delete", "selection-revert",
                        "selection-copy", "row-delete"] );
                    // selection-copy-down requires at least two rows the others at least 1
                    this._toggleEnableAction( lSelectedRecordIds.length >= 2, ["selection-copy-down"] );
                } else {
                    // In row selection mode
                    lSelectedRecords = lThisView.getSelectedRecords();
                    lEnabled = this._isCurrentlyEditable() && lSelectedRecords.length !== 0;
    
                    // these all require at least one row selected. 
                    this._toggleEnableAction( lEnabled, ["selection-duplicate", "selection-refresh", "selection-delete", "selection-revert",
                        "selection-copy", "selection-fill", "selection-clear"] );
                    ...
                    this._toggleEnableAction( lSelectedRecords.length >= 2, ["selection-copy-down"] );
                }
            }
        },
